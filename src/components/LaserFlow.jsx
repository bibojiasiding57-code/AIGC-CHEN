...
export const LaserFlow = ({
  ...
  const documentVisibilityPausedRef = useRef(false);
  const contextLossPausedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    documentVisibilityPausedRef.current = document.hidden;
    contextLossPausedRef.current = false;
    ...
    const isPaused = () => documentVisibilityPausedRef.current || contextLossPausedRef.current;
    ...
    const onCtxLost = event => {
      event.preventDefault();
      contextLossPausedRef.current = true;
      mount.dataset.laserState = 'fallback';
    };
    const onCtxRestored = () => {
      contextLossPausedRef.current = false;
    ...
    const animate = () => {
      ...
      if (isPaused() || !inViewRef.current) return;
    ...
    return () => {
      ...
      pointerTarget.removeEventListener('pointerenter', onMove);
  ...
  }, [dpr, pointerTargetRef, mouseSmoothTime]);
